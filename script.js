...
document.addEventListener("DOMContentLoaded", () => {
  const lines = document.querySelectorAll(".signature-line");

  lines.forEach((line, index) => {
    const key = "assinatura_" + index;

    const savedSpan = document.createElement("span");
    savedSpan.className = "signature-text";
    savedSpan.textContent = localStorage.getItem(key) || "";
    line.appendChild(savedSpan);

    line.addEventListener("click", (e) => {
      e.stopPropagation();

      if (line.querySelector(".signature-input")) return;

      const input = document.createElement("input");
      input.type = "text";
      input.className = "signature-input";
      input.value = savedSpan.textContent;
