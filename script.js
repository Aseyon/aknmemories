...
const captions = [
  ...
  'Descrição 5','Descrição 6','Descrição 7','Descrição 8'
];
...
prevThumb && prevThumb.addEventListener('click', ()=> showPhoto((current-1+8)%8));
nextThumb && nextThumb.addEventListener('click', ()=> showPhoto((current+1)%8));
...
(function(){
  ...
  if(!svg||!path) return;
  const W = 1600, H = 500, baseY = 390, strands = 60;
  ...
  function generate(t){
    ...
    if(dt>80){ influence*=0.94;if(influence<0.01) influence=0; }
...
function truncateCaption(caption) {
  ...
  caption.innerText = full.length > MAX_CHARS ? full.substring(0, MAX_CHARS) + "..." : full;
...
portraits.forEach(p => {
  p.addEventListener('click', (e) => {
    startMusic(); // <- inicia a música aqui
...
function centerBook() {
  ...
  let scale = 1;
  if(opened){
    // se você tiver uma animação que dobra o tamanho, você pode aplicar um scale aqui
...
function animateCatArms(action) {
  ...
  if(action === 'open') catArms.style.animation = 'arm-open 0.5s cubic-bezier(.22,.9,.33,1) forwards';
  else if(action === 'flip') catArms.style.animation = 'arm-flip 0.5s cubic-bezier(.22,.9,.33,1) forwards';
  else if(action === 'close') catArms.style.animation = 'arm-close 0.5s cubic-bezier(.22,.9,.33,1) forwards';
}
...
function closeBook() {
  ...
  setTimeout(()=>{
    ...
    // **Centraliza o livro de volta ao estado normal**
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const blurPush = 20;  // compensação do blur

  const radius = portalRadius - (particleSize / 2) + blurPush;
  // exemplo: 190 - 35 + 20 = 175px
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function closeGame() {
    ...
    // reseta iframe do jogo para parar som interno
    frame.src = "";
  }
